...
vix_sma_period = int(input("Enter the period of the VIX SMA: "))

weeks_list = []
for file in os.listdir('spy_daily_performance'):
    df_spy_performance = pd.read_csv(f'spy_daily_performance/{file}')
    new_week = True
...
for file in os.listdir('vix_daily_performance'):
    df_vix = pd.read_csv(f'vix_daily_performance/{file}')
    for i in range(1, len(df_vix.index)):
        current_date = datetime.date(int(df_vix['Date'][i].split('-')[0]), int(df_vix['Date'][i].split('-')[1]),
                                         int(df_vix['Date'][i].split('-')[2]))

        for week in weeks_list:
            for day in week.days_list:
                if current_date == day.date:
                    numerator = 0
                    for j in range(i - (vix_sma_period - 1), i + 1):
                        numerator += float(df_vix['Close'][j])

                    day.vix_sma = numerator / 50.0
                    day.daily_vix = float(df_vix['Close'][i])
...
for week in weeks_list:
    ...
    for day in week.days_list:
        if day.weekday_str == purchase_day and len(day.call_option_chain['strike']) != 0:
            ...
            print(f'VIX {vix_sma_period} SMA: {day.vix_sma}')
            print(f'VIX Dialy Value: {day.daily_vix}')
